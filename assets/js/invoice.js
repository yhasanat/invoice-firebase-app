...
async function saveCurrentInvoice(type, paymentOnly = false){
  ...
  if(res.status === "success"){
    alert(paymentOnly ? "تم تسجيل الدفعة في Firebase." : "تم حفظ الفاتورة في Firebase.");
  } else {
    queueInvoiceForSync(AppState.currentInvoice);
    alert("تم حفظ الفاتورة محلياً (أوفلاين). سيتم إرسالها إلى Firebase عند توفر الإنترنت.");
  }
