/* -------------------------------------------------
   data.js – إدارة البيانات عبر Firebase + مخزون ديناميكي
---------------------------------------------------*/
...
/* تحميل الأصناف – بدون الاعتماد على عمود stock */
async function loadProducts() {
  try {
    const rows = await firebaseGetCollection("Products");
...
async function loadCustomers() {
  try {
    const rows = await firebaseGetCollection("Customers");
    if (Array.isArray(rows) && rows.length) {
...
async function loadInvoices() {
  try {
    const inv = await firebaseGetCollection("Invoices");
    const items = await firebaseGetCollection("InvoiceItems");
...
async function calculateAllStocks(){
  const adds = await firebaseGetCollection("StockUpdates");
  const sales = await firebaseGetCollection("InvoiceItems");
  const stockMap = {};
...
/* حفظ فاتورة إلى Firebase + تسجيل حركة بيع في StockUpdates */
async function saveInvoiceToDB(invoice) {
  try {
    await firebaseAdd("Invoices", {
    ...
    for (const line of (invoice.items || [])) {
      await firebaseAdd("InvoiceItems", {
      ...
      await firebaseAdd("StockUpdates", {
        barcode: line.barcode,
        changeQty: -line.qty,
        source: "sale",
...
async function saveProductToDB(data) {
  try {
    ...
    // تحديث/إضافة بيانات الصنف (اختياري لكنه مفيد عند الاعتماد الكامل على Firebase)
    await firebaseAdd("Products", {
      barcode: data.barcode,
      name: data.name,
